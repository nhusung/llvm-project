...
cl::opt<bool> ExploreDivVF("explore-divide-by-vf", cl::Hidden, cl::init(false),
...
void createNewLoopFunction(std::unique_ptr<Module> *M, Function *OldFunc,
  ...
  LoopFunction->setAttributes(AttributeList::get(
      LoopFunction->getContext(),
      NewAttrs.getFnAttributes(),  // OldAttrs.getFnAttributes(),
      NewAttrs.getRetAttributes(), // OldAttrs.getRetAttributes(),
      NewArgAttrs));
...
PreservedAnalyses ExplorativeLVPass::run(Function &F,
  ...
  /******* Code from BackendUtil::EmitAssemblyWithNewPassManager ******/
  ...
  PassBuilder NewPB(false, TM, PTO, PGOOpt, &PIC);
  ...
  ModulePassManager ExplorativeMPM =
      NewPB.buildPerModuleDefaultPipeline(PassBuilder::OptimizationLevel::O3);
